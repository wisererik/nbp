...
func (p *Plugin) NodeExpandVolume(
	...
	*csi.NodeExpandVolumeResponse, error) {
	...
	// This funcationality to be filled
	...
	glog.V(5).Infof("end to node expand volume, args: %v", args)
	output, err := connector.ExecCmd("findmnt", args...)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "Could not determine device path: %v", err)
	}

	glog.V(5).Infof("end to node expand volume, mounted: %v", output)
	devicePath := strings.TrimSpace(string(output))
	if len(devicePath) == 0 {
		return nil, status.Errorf(codes.Internal, "Could not get valid device for mount path: %q", req.GetVolumePath())
	}
	glog.V(5).Infof("end to node expand volume, devicePath: %v", devicePath)

	var result bool
	if result, err = resize(devicePath, req.GetVolumePath()); err != nil {
		return nil, status.Errorf(codes.Internal, "Could not resize volume %q (%q):  %v", volumeID, devicePath, err)
	}

	glog.V(5).Infof("end to node expand volume, volume_size: %v, resizeresult: %v", volSizeBytes, result)
...
func resize(devicePath string, deviceMountPath string) (bool, error) {
	...
	glog.V(5).Infof("ResizeFS.Resize - format: %v", format)
	switch format {
	case "ext3", "ext4":
...
func  extResize(devicePath string) (bool, error) {
	...
	glog.V(5).Infof("end to node expand volume, hostId: %v", hostId)
	...
	glog.V(5).Infof("end to node expand volume, deviceIndex: %v", deviceIndex)
	...
	glog.V(5).Infof("end to node expand volume, reScanCommand: %v", reScanCommandPara)
